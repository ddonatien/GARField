...
class NeuSRenderer:
    ...
    def cat_z_vals(self, rays_o, rays_d, z_vals, new_z_vals, sdf, meshes, last=False):
        batch_size, n_samples = z_vals.shape
        ...
        if not last:
            new_sdf = self.sdf_network.sdf(pts.reshape(-1, 3), data_dict).reshape(
    ...
    def render_core(
        ...
        if self.aug_dirs and self.training:
            ids = torch.rand(dirs.shape[0], device=dirs.device, dtype=dirs.dtype)
            fill_dim = (torch.nonzero(ids < self.aug_dirs).shape[0], 3)
            fill = torch.normal(
                torch.zeros(fill_dim, device=dirs.device, dtype=dirs.dtype),
                torch.ones(fill_dim, device=dirs.device, dtype=dirs.dtype),
            )
            fill = fill / torch.norm(fill, dim=-1).unsqueeze(-1)
            dirs[ids < self.aug_dirs] = fill

        data_dict = {"meshes": meshes}
        if details is None:
            details = {"dists_gt": None, "partial_sdf": None}
        else:
            details["dists_gt"] = None
            details["partial_sdf"] = None

        sdf_nn_output, masks = sdf_network(pts, data_dict, details)
        sdf = sdf_nn_output[:, :1]
        mesh_sdf_error = 0
        for i, gt in enumerate(details["dists_gt"]):
            if gt is not None:
                mesh_sdf_error += F.mse_loss(details["partial_sdf"][i], gt)

        feature_vector = sdf_nn_output[:, 1:]

        gradients = sdf_network.gradient(pts, data_dict).squeeze()
        ...
        # depth map
        depth_map = torch.sum(weights_out * mid_z_vals, -1, keepdim=True)
        masks_out = (
            masks.reshape(mid_z_vals.shape[0], mid_z_vals.shape[1], -1)
            * weights_out[:, :, None]
        ).sum(dim=1)
        if self.use_color:
    ...
    def render(
        ...
        if self.n_importance > 0:
            with torch.no_grad():
                ...
                sdf = self.sdf_network.sdf(pts.reshape(-1, 3), data_dict).reshape(
        ...
        return {
            ...
            "masks": ret_fine["masks"],
            "mesh_sdf_error": ret_fine["mesh_sdf_error"],
        }

    # Depth
    def renderondepth(self, rays_o, rays_d, rays_s, meshes):
        pts = rays_o + rays_s  # n_rays, 3
        dirs = rays_d

        data_dict = {"meshes": meshes}

        sdf_nn_output, _ = self.sdf_network(pts, data_dict)
        feature_vector = sdf_nn_output[:, 1:]

        gradients = self.sdf_network.gradient(pts, data_dict).squeeze()
        sampled_color = self.color_network(pts, gradients, dirs, feature_vector)

        return sampled_color, gradients

    # Depth
    def errorondepth(self, rays_o, rays_d, rays_s, mask, meshes):
        pts = rays_o + rays_s
        ...
        true_cos = (rays_d * gradients).sum(-1, keepdim=True)
        relu_cos = F.relu(true_cos)
        pts = pts.detach()
        pts_norm = torch.linalg.norm(pts, ord=2, dim=-1, keepdim=True)
        # Denoise. not use: out of mask or sphere
        inside_masksphere = (pts_norm < 1.0).float() * mask  # inside_sphere * mask
        sdf = inside_masksphere * sdf
        inside_masksphere_sum = inside_masksphere.sum() + 1e-5
        sdf_error = (
            F.l1_loss(sdf, torch.zeros_like(sdf), reduction="sum")
            / inside_masksphere_sum
        )

        return sdf_error, inside_masksphere
